...
- the Genie Agent (or table set) to use for natural-language questions
- SQL Warehouse, if not already configured by the connector

If the user does not already have a Genie Agent, ask whether they want to continue with SQL-only dashboard and previews, configure a Genie Agent in Databricks first, or use the PAT fallback for direct Genie API access if available.
...
3. Ask for catalog, schema, tables, and Genie Agent.
