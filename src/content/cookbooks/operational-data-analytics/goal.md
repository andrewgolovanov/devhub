...
2. **Create a Lakebase Project** — provision a managed Postgres project as the OLTP source.
3. **Lakebase Change Data Feed (CDF)** — enable continuous replication from Lakebase tables to Unity Catalog Delta history tables.
4. **Medallion Architecture from CDC** — build silver (current-state) and gold (analytical) layers from the CDC history tables using Lakeflow Spark Declarative Pipelines.
