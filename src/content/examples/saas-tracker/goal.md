This template demonstrates a straightforward internal CRUD tool built on Databricks: a SaaS subscription tracker where teams log the tools they use, who owns each subscription, what it costs, and when it renews. A Genie Agent provides self-serve analytics over the subscription data.
...
4. A **Genie Agent** configured over the subscriptions table lets users ask natural language questions about spend, owners, and renewals.
...
- **Genie Agent**: Create a Genie Agent over the `saas_tracker.subscriptions` table and set the space ID.
