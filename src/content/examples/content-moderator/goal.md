...
Content moves through a review pipeline backed by Lakebase and Model Serving:

1. **Authors submit content** to Lakebase Postgres, specifying a title, body, and content target (blog, LinkedIn, etc.).
2. **AI scoring** triggers automatically. The server fetches active guidelines for the content target, sends the content plus guidelines to a Model Serving endpoint, and stores the compliance score (0-100), flagged issues, and improvement suggestions.
...
6. A **Genie Agent** over the content moderation tables enables natural language questions about content performance.
