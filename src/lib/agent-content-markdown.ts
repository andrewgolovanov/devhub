...
function resolveContentRoot(rootDir: string): string {
  return resolve(rootDir, "src", "content");
}

function resolveDocsRoot(rootDir: string): string {
  return resolve(rootDir, "src", "content", "docs");
}

function readDocsMarkdown(rootDir: string, slug: string): string {
  const docsDir = resolveDocsRoot(rootDir);
...
function readExampleMarkdown(rootDir: string, slug: string): string {
  ...
  const includedTemplates = [
    ...
    (
      template,
    ): template is (typeof cookbooks)[number] | (typeof recipes)[number] =>
      template !== undefined,
  );
...
function readCookbookMarkdown(
  ...
  if (goal === undefined) {
    throw new Error(
      `Missing required goal.md for cookbook "${slug}" at src/content/cookbooks/${slug}/`,
    );
  }
  ...
  return buildCookbookMarkdownDocument({
    ...
    cookbookUrl: `${siteOrigin.replace(/\/$/, "")}/templates/${cookbook.id}`,
    goal,
...
function readTemplateMarkdown(
  ...
  if (cookbook) {
    return readCookbookMarkdown(rootDir, slug, siteOrigin);
  }
...
export function getDetailMarkdown(
  ...
  switch (section) {
    ...
    case "examples":
      return readExampleMarkdown(rootDir, slug);
    case "templates":
      return readTemplateMarkdown(rootDir, slug, siteOrigin);
    default:
...
export function loadAgentPromptParts(
  ...
  const readContent = (slug: string): string =>
    readFileSync(resolve(contentRoot, `${slug}.md`), "utf-8");
  return {
 ...
 * + local-bootstrap + body). Used by route handlers to serve `.md` URLs and
 * the bootstrap prompt endpoint reuses the same shared composer.
 *
